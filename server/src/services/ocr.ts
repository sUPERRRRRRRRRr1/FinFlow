...
- "ทิศทาง" = ทิศทางเงินเทียบกับ "เจ้าของสลิป": ตอบ "out" ถ้าเจ้าของสลิปเป็นผู้ "โอนออก/จ่าย/ชำระ/เติมเงิน/ซื้อ" (กรณีปกติของสลิปเกือบทั้งหมด); ตอบ "in" เฉพาะใบที่เงินเข้าบัญชีเจ้าของสลิปจริง เช่น "เงินเข้า/รับโอนเข้าบัญชี/เงินเดือนเข้า"
- ระวัง: คำว่า "ชื่อผู้รับเงิน" หรือ "ผู้รับเงิน" บนสลิป = ปลายทางที่เรา "โอนไปให้" → เป็น "out" ไม่ใช่ "in"; เมื่อไม่แน่ใจให้ตอบ "out"
...
function jsonToSlipText(raw: string): string | null {
  ...
  // ทิศทางที่โมเดลสรุป → normalize เป็น out/in (ดีฟอลต์ out) ส่งให้ parseSlip เป็น marker
  // โมเดลตอบ "in"/"out" ตรง ๆ — ไม่ใช้ .includes('รับ') เพราะชนกับ "ผู้รับ" (ปลายทางของสลิป "จ่ายออก")
  const dirRaw = (j['ทิศทาง'] || '').toLowerCase();
  const dir = dirRaw === 'in' || dirRaw.includes('เข้า') ? 'in' : 'out';
  return [

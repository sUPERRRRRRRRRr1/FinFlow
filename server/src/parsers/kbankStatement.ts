...
export function parseSavingsStatement(text: string, source: Source = 'kbank'): Transaction[] {
  ...
  const re =
    /(\d{2})-(\d{2})-(\d{2})\s+(\d{2}):(\d{2})\s+(.+?)\s+([\d,]+\.\d{2})\s+(.+?)\s+(รับโอนเงิน|ฝากเงิน|โอนเงิน|ชำระเงิน|ถอนเงิน)\s+([\d,]+\.\d{2})/gu;
  ...
  while ((m = re.exec(normalized)) !== null) {
    ...
    const direction: Direction = type === 'รับโอนเงิน' || type === 'ฝากเงิน' ? 'in' : 'out';
    out.push(
      makeTxn({

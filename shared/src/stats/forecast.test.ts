...
describe('forecastByCategory — ตัดเดือนปัจจุบันที่ยังไม่จบ', () => {
  const txns: Transaction[] = [];
  // ม.ค.–พ.ค. 2025: อาหาร 3000/เดือน (เดือนเต็ม)
  for (let m = 1; m <= 5; m++) {
    for (let d = 1; d <= 10; d++) {
      txns.push(tx({ date: `2025-0${m}-${String(d).padStart(2, '0')}`, amount: 300, category: 'food' }));
    }
  }
  // มิ.ย. 2025 (เดือนปัจจุบัน ยังไม่จบ): อาหารแค่ 300
  txns.push(tx({ date: '2025-06-05', amount: 300, category: 'food' }));

  it('forecast อิงเดือนที่จบแล้ว (~3000) ไม่ถูกดึงต่ำตามเดือนปัจจุบัน', () => {
    const result = forecastByCategory(txns, [], 3, '2025-06-15');
    const food = result.categories.find((c) => c.category === 'food')!;
    expect(food.months[0]!.total).toBeGreaterThan(2000);
  });

  it('history จบที่เดือนล่าสุดที่เต็ม (พ.ค.) ไม่รวมเดือนปัจจุบัน (มิ.ย.)', () => {
    const result = forecastByCategory(txns, [], 3, '2025-06-15');
    const food = result.categories.find((c) => c.category === 'food')!;
    const labels = food.history.map((h) => h.label);
    expect(labels[labels.length - 1]).toContain('พ.ค.');
    expect(labels.some((l) => l.includes('มิ.ย.'))).toBe(false);
  });
});
